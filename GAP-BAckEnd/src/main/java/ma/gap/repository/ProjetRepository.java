...
public interface ProjetRepository extends JpaRepository<Projet, Long> {
        ...
        @Query(value = "SELECT DISTINCT p.* FROM projet p " +
                        ...
                        "WHERE a.id = :atelier_id  AND art.quantite_tot > art.quantite_en_prod AND p.status=2 ", nativeQuery = true)
        List<Projet> findAffairesByAtelierAndQteArticle_Sup_QteEnProd(@Param("atelier_id") long atelier_id);

        @Query("SELECT DISTINCT p FROM Projet p, OrdreFabrication odf WHERE p.id=odf.projet.id and " +
                        "odf.atelier.id = :atelierId and odf.qteRest > 0 order by p.id desc")
        List<Projet> findAffairesByAtelierAndOfDeliverable(@Param("atelierId") long atelierId);
}

...
public class ArticleController {
    ...
    public ResponseEntity<?> findAllArticles(@PathVariable("idUser") long idUser) {
        try {
            ...
            if (isAdminOrLogistique) {
                articles=articleService.findAll(Sort.by(Sort.Direction.DESC, "id"));
            } else {
                articles = articleService.allArticleByAtelier(user.getAteliers().get(0).getId());
            }
    ...
    public ResponseEntity<?> save(@RequestBody Article article) {
        try {
          ...
          articleService.saveArticle(article,idArticle);
            ...
            return ResponseEntity.status(HttpStatus.CREATED).body("Article enregistré avec succès.");  // Retourner l'ID dans la réponse
        } catch (Exception e) {
    ...
    public ResponseEntity<?> allArticleByProject(@RequestParam("idUser") long idUser,@RequestParam("idprojet") long idprojet) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Erreur lors de la récupération des articles.");
        }
    }
    ...
    @GetMapping("/search")
    public List<Article> searchArticles(@RequestParam(value = "idUser") long idUser, @RequestParam("numPrix") String numPrix,@RequestParam("designation") String designation, @RequestParam("idprojet") long idProjet, @RequestParam("idatelier") long idAtelier, @RequestParam("idarticle") long idArticle) throws ParseException {
       List<Article> articles=articleService.searchArticle( idUser,  numPrix,  designation,  idProjet,  idAtelier,  idArticle);
        articles.sort(Comparator.comparing(Article::getId).reversed());
        return articles;
    }
    @GetMapping(value = "/findArticles_QteSup_QteOF")
    public ResponseEntity<?> findArticles_QteSup_QteOF(@RequestParam("idprojet") long idprojet,@RequestParam("idatelier") long idatelier) {
        try {
